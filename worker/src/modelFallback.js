...
export async function callModelWithFallback(requestedModelId, payload, env, userPlanId) {
  ...
  // If we exhaust the fallback chain, return the last failure
  if (env.SENTRY_DSN) {
    Sentry.addBreadcrumb({
       category: 'model-fallback-exhausted',
       message: `All eligible models failed for request starting with ${requestedModelId}.`,
       level: 'error'
    });
  }
  return { ok: false, response: lastResult?.response, error: lastResult?.error, servedByModel: null };
}
