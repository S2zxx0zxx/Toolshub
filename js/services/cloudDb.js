...
export const CloudDB = (() => {
  ...
  function subscribeConversations(callback) {
    ...
    const q = query(
      collection(db, `users/${_uid()}/conversations`), 
  ...
  async function loadChatMessages(chatId) {
    const q = query(
      collection(db, `users/${_uid()}/conversations/${chatId}/messages`),
      orderBy('timestamp', 'asc')
    );
    const snap = await getDocs(q);
    const messages = [];
    snap.forEach(doc => {
      // Map Firestore structure back to app structure
      const data = doc.data();
      messages.push({ 
        id: doc.id, 
        role: data.role, 
  ...
  async function saveConversation(chat) {
    ...
    const convRef = doc(db, `users/${_uid()}/conversations`, id);
    
    // To respect the serverTimestamp rule and avoid overwriting existing createdAt:
  ...
  async function saveMessage(chatId, message) {
    ...
    await setDoc(doc(db, `users/${_uid()}/conversations/${chatId}/messages`, msgId), {
      role: message.role,
    ...
    await setDoc(doc(db, `users/${_uid()}/conversations`, chatId), {
      updatedAt: serverTimestamp(),
