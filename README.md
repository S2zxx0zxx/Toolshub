...
### 🛠️ Updates: Recent Stability & Data-Accuracy Pass (v5.1)
| File | What Changed |
| :--- | :--- |
| `index.html` | Embedded real UI images for landing page feature cards (Capabilities, Deep Research, Team Collab). Wired actual IDs to Billing panel. |
| `css/layout/home.css` | Fixed baked-in PNG margins on feature cards (`object-position: center bottom` + scale crop). |
| `worker/src/payments.js` | Razorpay webhook payload now extracts actual `paymentMethod` and card network/last4. |
| `js/core/app.js` | Wired real `CloudDB.getMonthlyUsage()` and `LocalSettings` subscription data to the UI billing panel. Fixed clear-history to instantly refresh sidebar without reload. |
| `js/ui/statusScreen.js` | Wired physical back-button via native `Router.navigate('settings')`. |
| `js/core/router.js` | Added native `history.pushState` and `popstate` support for app-wide back navigation. |
| `js/tools/registry.js` | Isolated `isFresh` context flag into `markToolContextUsed()` to prevent premature consumption by repeated polling in ChatEngine. |
| `js/ui/connectorsSheet.js` | Implemented real `fetch('https://api.github.com/user')` validation for GitHub tokens before marking as Connected. |
| `worker/src/index.js` | Fixed UptimeRobot 405 error on `/api/health` by explicitly allowing HTTP `HEAD` pings. |
| `worker/src/modelAccess.js` | Fixed incorrect model API path for GitHub's `gpt-4o-mini`. |
